...
import deleteBoothByBoothId from '../../helpers/booths/deleteBoothByBoothId'
...
const deleteBooth: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId && boothId) {
      const newData = await deleteBoothByBoothId(eventId.toString(), boothId.toString());
