import getBoothByBoothId from '../../helpers/booths/getBoothByBoothId';
...
const getBooth: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    const eventId = req.query.event_id || req.params.eventId;
    const boothId = req.query.booth_id || req.params.boothId;
    if (eventId && boothId) {
      const eventData = await getBoothByBoothId(eventId.toString(), boothId.toString());
      if (eventData) {
