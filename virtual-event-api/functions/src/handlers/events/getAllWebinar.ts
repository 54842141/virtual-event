import getWebinarByEventId from '../../helpers/webinars/getWebinarByEventId';
...
const getAllWebinar: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const eventData = await getWebinarByEventId(eventId.toString());
      if (eventData) {
