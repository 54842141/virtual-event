import getBannerContainerByEventId from '../../helpers/bannerContainers/getBannerContainerByEventId';
...
const getAllBannerContainer: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const eventData = await getBannerContainerByEventId(eventId.toString());
      if (eventData) {
...
export default getAllBannerContainer;
