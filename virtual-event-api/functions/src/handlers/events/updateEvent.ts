...
import updateEventByDomain from '../../helpers/events/updateEventByDomain'
...
const updateEvent: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const newData = await updateEventByDomain(eventId.toString(), req.body);
