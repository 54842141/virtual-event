import addEventByEventDomain from '../../helpers/events/addEventByEventDomain';
...
const addEvent: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventName) {
      const eventData = await addEventByEventDomain(req.body);
      if (eventData) {
