import getBoothByEventId from '../../helpers/booths/getBoothByEventId';
...
const getAllBooth: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const eventData = await getBoothByEventId(eventId.toString());
      if (eventData) {
