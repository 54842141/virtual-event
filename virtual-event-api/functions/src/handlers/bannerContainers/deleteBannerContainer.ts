...
import deleteBannerContainerByBannerContainerId from '../../helpers/bannerContainers/deleteBannerContainerByBannerContainerId';
...
const deleteBannerContainer: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId && bannerContainerId) {
      const newData = await deleteBannerContainerByBannerContainerId(eventId.toString(), bannerContainerId.toString());
