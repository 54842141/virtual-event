import getBannerContainerByBannerContainerId from '../../helpers/bannerContainers/getBannerContainerByBannerContainerId';
...
const getBannerContainer: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId && bannerContainerId) {
      const eventData = await getBannerContainerByBannerContainerId(eventId.toString(), bannerContainerId.toString());
      if (eventData) {
