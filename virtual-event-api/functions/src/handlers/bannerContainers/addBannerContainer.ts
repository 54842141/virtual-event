import addBannerContainerByEventId from '../../helpers/bannerContainers/addBannerContainerByEventId';
...
const addBannerContainer: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const eventData = await addBannerContainerByEventId(req.body);
      if (eventData) {
