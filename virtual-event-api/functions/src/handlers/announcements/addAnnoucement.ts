import addAnnoucementByEventId from '../../helpers/announcement/addAnnoucementByEventId';
...
const addAnnoucement: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId) {
      const eventData = await addAnnoucementByEventId(req.body);
      if (eventData) {
