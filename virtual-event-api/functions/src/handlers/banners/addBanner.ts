...
import addBannerByBannerContainerId from '../../helpers/banners/addBannerByBannerContainerId';
...
const addBanner: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    const { bannerContainerId } = req.body;
    if (bannerContainerId) {
      const bannerData = await addBannerByBannerContainerId(req.body);
      if (bannerData) {
        setResponse(res, 200, bannerData);
      } else {
