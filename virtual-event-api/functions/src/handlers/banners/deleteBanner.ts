...
import deleteBannerByBannerId from '../../helpers/banners/deleteBannerByBannerId';
...
const deleteBannerContainer: RequestHandler = async (
  ...
  try {
    ...
    if (bannerId && bannerContainerId) {
      const newData = await deleteBannerByBannerId(bannerContainerId.toString(), bannerId.toString());
