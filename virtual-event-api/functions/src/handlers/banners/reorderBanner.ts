...
import reoderBannerByBannerContainerId from '../../helpers/banners/reoderBannerByBannerContainerId'
...
const reorderBanner: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (bannerContainerId) {
      const newData = await reoderBannerByBannerContainerId(bannerContainerId.toString(), req.body);
