import getBannerContainerByBannerContainerId from '../../helpers/banners/getBannersByBannerContainerId';
...
const getBannerContainer: RequestHandler = async (
  ...
  try {
    ...
    if (bannerContainerId) {
      const eventData = await getBannerContainerByBannerContainerId(bannerContainerId.toString());
      if (eventData) {
