...
import deleteWebinarByWebinarId from '../../helpers/webinars/deleteWebinarByWebinarId'
...
const deleteWebinar: RequestHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (eventId && webinarId) {
      const newData = await deleteWebinarByWebinarId(eventId.toString(), webinarId.toString());
